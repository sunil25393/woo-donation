...
function sp_edp_enqueued_style() {
    if (is_admin()) {
        wp_enqueue_style('spwd_custom', spwd_plugin_url('css/sp_edp_custom.css'), array(), CCF_VERSION, 'all');
    }
    $woocommerce->session->sp_donation = woo_donations_get_saved_strings_admin('donation_minimum_amount');
}
...
function woo_donation_submenu_callback() {
    ...
    <div class="donation_admin"><h3>Woo Donation</h3>
        ...
        <div class="Information clearfix">
            ...
            <p class="description"><h2><?php echo __("For any kind of query, You can leave"); ?> <a href="www.sunilprajapati.in/woo-donation/"><?php echo __("Comment here"); ?></a></h2></p>
            <p class="description"><h2><?php echo __("For full documation of Plugin"); ?> <a href="www.sunilprajapati.in/woo-donation/"><?php echo __("Click here"); ?></a></h2></p>
            <p class="description"><h2> Rate Us :  <a href="">Click Here</a></h2></p>
            <p></p>
        </div>
    </div>
    <?php
...
if (get_option('donation_table_position') == 'after') {
        ...
        if (!donation_exists()) {
            ...
            <tr class="donation-block">
                ...
                <?php if (woo_donations_get_saved_strings_admin('donation_minimum_amount') != '') { ?>
                    <form action="" method="post" onsubmit="return donationvalidationEvent()">
                    <?php } else { ?>
                        <form action=""method="post">   
                        <?php } ?>
...
function sp_checkout_field_process() {
    ...
    $md = woo_donations_get_saved_strings_admin('donation_mandatory');
    if ($md == 'yes') {
        if (!donation_exists()) {
